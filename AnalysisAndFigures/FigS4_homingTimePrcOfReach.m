...
%%% reaction time, midreach time, and homing time. We then look at each
%%% (specifically homing time) as a fraction of the total reach epoch time
%%% (calculated as their sum).
%%%
%%% Adam Smoulder, 10/16/20
...
reactionTimePrc_bySubject_byReward = cell(nsubjects,nrewards);
midreachTimePrc_bySubject_byReward = cell(nsubjects,nrewards);
homingTimePrc_bySubject_byReward = cell(nsubjects,nrewards);
...
for f = 1:nsubjects
    ...
    for r = 1:nrewards
        ...
        reactionTimePrc_bySubject_byReward{f,r} = 100*reactionTime(curInds)./reachTime_bySubject_byReward{f,r};
        midreachTimePrc_bySubject_byReward{f,r} = 100*midreachTime(curInds)./reachTime_bySubject_byReward{f,r};
        homingTimePrc_bySubject_byReward{f,r} = 100*homingTime(curInds)./reachTime_bySubject_byReward{f,r};
    end; clear r
end; clear f

n_bySubject_byReward = cellfun(@(x) length(x), reactionTimePrc_bySubject_byReward)
nBadHalfway_bySubject_byReward = cellfun(@(x) sum(isnan(x)), midreachTimePrc_bySubject_byReward)
nBadHoming_bySubject_byReward = cellfun(@(x) sum(isnan(x)), homingTimePrc_bySubject_byReward)

%% NOTE homing times are NOT normally distributed
figure; qqplot(homingTimePrc_bySubject_byReward{1,2})
...
for f = 1:nsubjects
    for r = 1:nrewards
            curVals = homingTimePrc_bySubject_byReward{f,r};
...
for f = 1:nsubjects
    ...
    yticks([19 29])
    if f == 1
        ylabel('Homing Time (% of reach time)')
    else
        yticks([])
    end
    axis([rewards(1)-1 rewards(end)+1 18 30])
    title(['Monkey ' subjectNames{f}(1)])
end; clear f
set(gcf,'position',[33 638 895 319])

% Save it!
figname = 'FigS4_homingTimePrcOfReach';
...
for f = 1:nsubjects
    for r1 = 1:nrewards
        for r2 = 1:nrewards
            pvals_byRewardXReward_bySubject(r1,r2,f) = ranksum(homingTimePrc_bySubject_byReward{f,r1},homingTimePrc_bySubject_byReward{f,r2});
        end; clear r2
    end; clear r1
end; clear f
